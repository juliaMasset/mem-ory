# Step 3.2 - ES6 Arrow functions

topics: **`Function.bind()`**, **arrow functions**, **template literals**, **`const`**, **`let`**

Open `game.component.js`, and look at the `gotoScore()` method:

```javascript
// front-end/src/app/components/game/game.component.js
function gotoScore() {
  var now = Date.now();
  var timeElapsedInSeconds = Math.floor((now - this._startTime) / 1000);
  clearInterval(this._timer);

  setTimeout(
    function () {
      window.location =
        "../score/score.component.html?name=" +
        this._name +
        "&size=" +
        this._size +
        "&time=" +
        timeElapsedInSeconds;
    }.bind(this),
    750
  );
}
```

This methods is called once all pairs are flipped. It waits 750ms, then it redirects the player to the `ScoreComponent`.

You can enhance this code with a bunch of ES6+ features:

- ES5 used to define variables with the `var` keyword. Since ES6, `let` and `const` replace `var`, with a better handling of the [variable scope](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/let#examples) that may avoid nasty bugs.
  Whenever you see the `var` keywork, you want to replace it with either `let` or `const`.

  > ![question] What are the differences between `var` and `let`;

- Concatenate strings is not something we should enjoy in any language. Fortunately, ES6 offers an alternative for that, named [**template litterals**](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Template_literals).  
  **Instead of this:**
  ```javascript
  window.location =
    "../score/score.component.html?name=" +
    this._name +
    "&size=" +
    this._size +
    "&time=" +
    timeElapsedInSeconds;
  ```
  you'd better write this:
  ```javascript
  window.location = `../score/score.component.html?name=${this._name}&size=${this._size}&time=${timeElapsedInSeconds}`;
  ```
- Did you see that callback passed to `setTimeout()`?  
  With ES6's new [**Arrow functions**](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Functions/Arrow_functions), it becomes much more easier to write anonymous functions or callback functions.
  Rewrite this with the `() => { ... }` syntax.

  > ![question] What is the `.bind(this)` stuff? What does happen if you delete it? Is it needed when using an _arrow function_ ?

With the example above, you just learned to use `let`, `const`, _arrow functions_ and \*template litterals.

Now, apply the same changes all over the code.

**Your job**:

- Go through `game.component.js`, `welcome.component.js`, `card.component.js` and `score.component.js`
- replace `var` keywords with either `let` or `const`
- replace _callback_ functions with _arrow functions_
- replace string concatenations with _template litterals_
- search for `// TODO Step 3.2` comment to help you find out all the places code needs updates

#### Checklist

- [ ] I know the differences between `var` and `let`/`const`
- [ ] All `var` has been replaced with either `let` or `const`
- [ ] I know about template literals
- [ ] I can write arrow functions
- [ ] I know the value of `this` is not always as expected
- [ ] I refactored all `// TODO Step 3.2` that I found
- [ ] The application still runs as usual

**![commit] commit step**

[commit]: ../assets/commit.png
[success]: data:image/png;base64, "success"
[info]: data:image/png;base64, "info"
[question]: data:image/png;base64, "question"
[error]: data:image/png;base64, "error"
[warning]: data:image/png;base64, "warning"
[danger]: data:image/png;base64, "danger"
[tip]: data:image/png;base64, "tip"
[boom]: data:image/png;base64, "boom"
