# Step 0 - Setup

> ![info] This tutorial will work either on Windows, Linux or MacOS, but it is best suited for linux.

## 0.1 - Prerequisites

- Have a recent LTS version of [**nodejs**](https://nodejs.org/en/)

  ```sh
  $ node -v
  v14.16.1 ### at least
  ```

  > ![tip] **Pro tip**: You can optionally install [NVM](https://github.com/creationix/nvm). This is a very useful tool
  > that makes it easier to install and switch between different versions of nodeJS.

- Have a web browser with good capabilities for debugging javascript (Eg: Google Chrome, Chromium, or Firefox).

- A good IDE. I recommend [Visual Studio Code](https://code.visualstudio.com/) as it is free and web development.

> ![info] `-g` parameter installs a dependency **globally**. This way, the dependency will be available system-wide.

> ![danger] Linux systems by default requires root privileges to use the `-g` parameter, but I recommend you **NEVER EVER use `sudo npm install ...`**.
> TO work around this issue, I suggest you [set another NPM prefix](https://docs.npmjs.com/resolving-eacces-permissions-errors-when-installing-packages-globally)

## 0.2 Setup

In this tutorial, we start from an existing _Memory_ game, and enhance the codebase to create a _SPA_ written in _ESNext_.
The game is called _Meme-ory_, and it involves:

- A **Front-end**. This is HTML, CSS and JS the code for the user interface, and it requires an HTTP Server.
- Another server with an API to send game data to the front end. Call it the **Back-end**

The _meme-ory_ game you start from is made of 3 views:

- **the welcome view** (`WelcomeComponent`). It contains a simple form to let the player enter his name, the desired game size and a start button to launch the game.
- **the game view allowing** (`GameComponent`). This is where the player plays the game, flipping cards 2 by 2 until all the cards are turned upwards.
- **the score view** (`ScoreComponent`). It displays the player's score.

![game-mockup]

**Your job:**

- Init a new git repository for the project of this tutorial. Call it the way you want. Mine is called `meme-ory`.
- Copy the [`back-end/`]({{$env.GITLAB_URL}}/{{$m3.gitlab.slug}}/-/tree/main/resources/setup/back-end) and the [`front-end/`]({{$env.GITLAB_URL}}/{{$m3.gitlab.slug}}/-/tree/main/resources/setup/front-end) folders into the root of your repository.
- Start the back-end:
  ```bash
  cd back-end
  npm install
  npm start
  ```
- Ensure the server is up and running: Connect to [http://localhost:8081/api-docs/](http://localhost:8081/api-docs/) and check the documentation is alive.
  You should keep this back-end up and running during the whole tutorial.
- Start the HTTP server to serve the front-end:

  ```bash
  cd front-end
  npx http-server -c-1
  ```

- Ensure everything works as intended. Navigate to [localhost:8080/src](http://localhost:8080/src).
  You should be able to:
  - enter your name and start a new game (`WelcomeComponent`)
  - play and win the game (`GameComponent`)
  - get redirected to see your score (`ScoreComponent`)

> ![tip] **Did you know**? Your web browser can tell you a lot on what happens behind the scene (javascript execution, stack-traces, network, errors, ...). `ctrl+shift+I` (firefox, chrome, chromium) to access the developer console. There, you can open source files (`ctrl+P`), inspect HTML elements, and monitor the Network traffic.
> ![network-dev-tool]

> ![question] Play the whole game with `size=2`. By browsing the 3 views of the application, how many files did your browser download overall? How many time did it took to load them all?

#### Files produced:

```
back-end/...
front-end/src/index.html
front-end/src/app/scripts/...
front-end/src/app/styles/...
front-end/src/app/views/...
```

#### Checklist

- [ ] I can play meme-ory; The application looks 'OK' and works.
- [ ] My application is served at `localhost` with `http-server`
- [ ] I know how to open and how to use the developer console.

**![commit] commit step**

## 0.3 Troubleshoot

### Troubleshoot 1

> ![danger] When i run **_npx http-server -c-1_** command it produce an infinite loop on my standard output...

The _colors_ package (1.4.1 version) introduced an offending commit that adds an infinite loop to the source code.
Unfortunately, the _http-server_ package includes this version in its sub-dependencies.
To get rid of it, please apply the following steps :

- First, install a fixed colors package version, it will force _http-server_ to choose this one.

```
cd front-end/
npm install colors@1.4.0 --save --saved-exact
```

- Check that npm correctly created package*.json files and node_modules directory where the*colors\* package is located.

```bash
> ls

node_modules/ package.json package-lock.json src/
```

- Install the _http-server_ package

```bash
npm install http-server
```

- Check that _http-server_ package is correctly added to node_modules directory and package\*.json files. And finally, print your project dependencies tree with the following command (from the front-end directory)

```bash
 npm list --depth=2

front-end@ .../es6-01/meme-ory/front-end
├── colors@1.4.0
└─┬ http-server@14.0.0
  ├─┬ basic-auth@2.0.1
  │ └── safe-buffer@5.1.2
  ├── colors@1.4.0 deduped   #check this line
  ├── corser@2.0.1
   ...
```

- The _colors_ 1.4.0 version is know correctly added and used by _http-server_. Now, you can start the HTTP-server without troubles !

```bash
npx http-server -c-1
```

[game-mockup]: ../assets/mockup.png
[network-dev-tool]: ../assets/chrome-dev-tools-network.png
[commit]: ../assets/commit.png
[success]: data:image/png;base64, "success"
[info]: data:image/png;base64, "info"
[question]: data:image/png;base64, "question"
[error]: data:image/png;base64, "error"
[warning]: data:image/png;base64, "warning"
[danger]: data:image/png;base64, "danger"
[tip]: data:image/png;base64, "tip"
[boom]: data:image/png;base64, "boom"
[error]: data:image/png;base64, "error"
