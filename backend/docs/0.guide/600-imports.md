# Step 6 - Webpack & imports

Do you remember the `parseUrl` function at the bottom of `game.component.js` ?

```javascript
// front-end/src/app/components/game/game.component.js
function parseUrl() {
  // ...
}
```

Its purpose is to take all the parameters found in the URL (`window.location`), so the component can load them.
This function is needed in `GameComponent`, but also in `ScoreComponent`, and the way to deal with this at the moment is to duplicate the function implementation in both files.

With ES6, we now have the ability to write [modules](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/import) that `export` some features. Exported features can in turn be `import`-ed several times in other files.

In this step, we create a new `utils.js` file where `parseUrl()` function is defined once, and imported in `game.component.js` & `score.component.js`.

## Step 6.1 - import, export

In this step, we will create a new `main.js` file, to ensure `parseUrl` function works properly once imported.

**Your job**

- Create a new file called `front-end/src/app/utils/utils.js` file, and paste the `parseUrl` function as follows:
  ```javascript
  // front-end/src/app/utils/utils.js
  export function parseUrl() {
    // ... parseUrl implementation
    return result;
  }
  ```
  > ![info] The `export` keyword means the symbol `parseUrl` will be public to other modules. This way, it and can be imported by other files.
- Remove `function parseUrl()` from `game.component.js` and `score.component.js`, and instead, import the function from `utils.js` instead:
  ```javascript
  // front-end/src/app/components/game/game.component.js
  import { parseUrl } from "../../utils/utils";
  ```
  > ![info] When a file contains at least one `import` or one `export`, it is automatically considered as an ES6 module.
  > All the symbols defined within that file won't be part of the global scope, and will be encapsulated in that module.
- Because they use the `import` keyword, `game.component.js` and `score.component.js` are now considered as modules. As a result, you can drop the closure in these files;

```js
// front-end/src/app/components/game/game.component.js
import { parseUrl } from "../../utils/utils";

(function () /* <== not needed in ES modules*/ {
  // ...
})();
```

> ![question] What is the difference between `import * from './utils'` and `import { parseUrl } from './utils'`?

- Test the result: go to [http://localhost:8080/src/index.html?value1=someValue1&value2=someValue2](http://localhost:8080/src/index.html?value1=someValue1&value2=someValue2), and open your dev console
  > ![boom] **Uncaught SyntaxError: Cannot use import statement outside a module**

That's right: even if ES6 defines `import` and `export`, your browser won't be able to use them _as is_, and transpilers like Babel won't help you with this issue.

To tackle down this issue, we need another kind of tools called **Bundlers**.

Your project does not work anymore at the end of this step, but everything will be fixed in the next step with the introduction of Webpack.

#### Files produced:

```
front-end/src/app/utils/utils.js
```

#### Checklist

- [ ] I know how to write `imports` and `exports`

**![commit] commit step**

[commit]: ../assets/commit.png
[success]: data:image/png;base64, "success"
[info]: data:image/png;base64, "info"
[question]: data:image/png;base64, "question"
[error]: data:image/png;base64, "error"
[warning]: data:image/png;base64, "warning"
[danger]: data:image/png;base64, "danger"
[tip]: data:image/png;base64, "tip"
[boom]: data:image/png;base64, "boom"
