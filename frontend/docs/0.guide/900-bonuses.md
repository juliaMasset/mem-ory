...
## Step 9.1 LocalStorage

In this step, our goal is to preserve our game's state even if we exit the browser or the user reloads the page.
...
> ![tip] You can import localforage like this:
>
> ```javascript
...
all scores saved to let `ScoreComponent` print a beautifull **high-scores** top.
...
You can also compare your work with the [`done` branch]({{$m3.env.GITLAB_URL}}/master3/ES6-01/tree/done).

Ready to follow up? Get started for next [angular module 01]({{$m3.env.M3_PRESS_URL}}/angular-01) module.
